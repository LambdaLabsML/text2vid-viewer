...
def validate_and_process_csv(file_path):
    ...
    try:
        # Read the CSV file using the csv module to handle irregularities
        ...
        # Fill missing 'base_prompt' values with empty strings
        df['base_prompt'] = df['base_prompt'].fillna('')

        # Rest of your processing remains the same
        # Replace "/" and strip whitespace in both 'prompt' and 'base_prompt' columns
        df['prompt'] = df['prompt'].str.replace("/", "").str.strip()
        df['base_prompt'] = df['base_prompt'].str.replace("/", "").str.strip()
