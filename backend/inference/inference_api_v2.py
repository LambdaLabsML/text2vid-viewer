"""
Inference API with multi GPU support
"""
...
def generate_image_multiGPU(data):
    ...
    logging.debug(f"Running multi-GPU command: {' '.join(cmd)}")
    return cmd

def get_cmd_list(data, multi_gpu=False):
    """Prepare the command list for image generation."""
    if multi_gpu:
        return generate_image_multiGPU(data)
...
def generate_image():
    try:
        ...
        # Run inference cmd
        cmd_list = get_cmd_list(data, multi_gpu=False)  # Set multi_gpu=True to use multi-GPU
